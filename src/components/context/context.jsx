...
const DataProvider = props => {
    ...
    if (token) {
      try {
        ...
        if (json.status === 200) {
          ...
          lista.map(el => {
            el.fechaTabla = Moment(el.DocDate).format('DD/MM/YYYY HH:mm')
          })
          setDataPedidos(lista)
        } else if (json.status === 201) {
          setVisibleLoading(false)
          setDataPedidos([])
          setVisibleModal({
  ...
  const consultarProducts = async () => {
    ...
    try {
      const json = await axios.get(URL + 'articulos/consultarSiigo', {
        headers: { authorization: token }
      })
   ...
   *
   */
  const consultarTaxes = async () => {
    const token = localStorage.getItem('token')

    try {
      const json = await axios.get(URL + 'taxes/consultarSiigo', {
        headers: { authorization: token }
      })
      if (json.status === 200) return false
    } catch (e) {
   ...
   *
   */
  const consultarWarehouses = async () => {
    const token = localStorage.getItem('token')

    try {
      const json = await axios.get(URL + 'bodegas/consultarSiigo', {
        headers: { authorization: token }
      })
      if (json.status === 200) return false
    } catch (e) {
  ...
  const consultarClients = async () => {
    ...
    try {
      const json = await axios.get(URL + 'clientes/consultarSiigo', {
        headers: { authorization: token }
      })
      if (json.status === 200) return false
    } catch (e) {
  ...
  const llenarEncabezado = () => {
    setcolumnasPedidos([
      ...
      },
      {
        ...
        dataIndex: 'Usuario',
        key: 'Usuario',
        render:text=><div>{text.nombre}</div>
      },
      { title: 'Registrado', dataIndex: 'fechaTabla', key: 'fechaTabla' }
    ])
  }
